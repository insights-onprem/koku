...
class AWSReportDBAccessor(SQLScriptAtomicExecutorMixin, ReportDBAccessorBase):
    ...
    def populate_line_item_daily_summary_table_trino(self, start_date, end_date, source_uuid, bill_id, markup_value):
        ...
        """
        sql = pkgutil.get_data("masu.database", f"{self.get_sql_folder_name()}/reporting_awscostentrylineitem_daily_summary.sql")
        sql = sql.decode("utf-8")
    ...
    def populate_ocp_on_aws_ui_summary_tables_trino(
        ...
        for table_name in tables:
            sql = pkgutil.get_data("masu.database", f"{self.get_sql_folder_name()}/aws/openshift/{table_name}.sql")
            sql = sql.decode("utf-8")
            sql_params = {
                "schema": self.schema,
                "start_date": start_date,
    ...
    def get_openshift_on_cloud_matched_tags_trino(
        ...
        """Return a list of matched tags."""
        sql = pkgutil.get_data("masu.database", f"{self.get_sql_folder_name()}/reporting_ocpaws_matched_tags.sql")
        sql = sql.decode("utf-8")
    ...
    def populate_ec2_compute_summary_table_trino(self, source_uuid, start_date, bill_id, markup_value):
        ...
        sql = pkgutil.get_data("masu.database", f"{self.get_sql_folder_name()}/{table_name}.sql")
        sql = sql.decode("utf-8")
        sql_params = {
            "schema": self.schema,
            "source_uuid": source_uuid,
