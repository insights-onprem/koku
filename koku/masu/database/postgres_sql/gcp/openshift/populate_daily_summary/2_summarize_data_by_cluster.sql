DELETE FROM {{schema | sqlsafe}}.managed_reporting_ocpgcpcostlineitem_project_daily_summary_temp
    ...
    ON gcp.usage_start = ocp.usage_start
        AND (
                ...
                OR (gcp.matched_tag != '' AND EXISTS (
                    SELECT 1
                    FROM unnest(string_to_array(gcp.matched_tag, ',')) AS matched_tag_elem
...
{%- endif -%}
INSERT INTO {{schema | sqlsafe}}.managed_reporting_ocpgcpcostlineitem_project_daily_summary (
    row_uuid,
    cluster_id,
    cluster_alias,
    data_source,
...
WITH cte_rankings AS (
    ...
    FROM {{schema | sqlsafe}}.managed_reporting_ocpgcpcostlineitem_project_daily_summary_temp AS pds
...
SELECT gcp.row_uuid as row_uuid,
    ...
    {{cloud_provider_uuid}} as source,
    {{ocp_provider_uuid}} as ocp_source,
    EXTRACT(YEAR FROM max(gcp.usage_start))::text as year,
    EXTRACT(MONTH FROM max(gcp.usage_start))::text as month,
