...
JOIN public.reporting_common_diskcapacity as az_disk_capacity
...
WHERE azure.date >= '{{start_date | sqlsafe}}'::timestamp
    AND azure.date < '{{end_date | sqlsafe}}'::timestamp + INTERVAL '1 day'
...
SELECT uuid_generate_v4()::text as row_uuid,
    ...
    JOIN {{schema | sqlsafe}}.managed_azure_openshift_disk_capacities_temp AS az_disk
...
SELECT azure.row_uuid as row_uuid,
    ...
    LEFT JOIN {{schema | sqlsafe}}.managed_azure_openshift_disk_capacities_temp as disk_cap
...
SELECT azure.row_uuid,
    ...
    JOIN {{schema | sqlsafe}}.managed_azure_openshift_daily_temp as azure
        ON azure.usage_start = ocp.usage_start
        AND (
                azure.tags::json->>'openshift_project' = ocp.namespace
                OR azure.tags::json->>'openshift_node' = ocp.node
                OR azure.tags::json->>'openshift_cluster' = ocp.cluster_alias
                OR azure.tags::json->>'openshift_cluster' = ocp.cluster_id
                OR (azure.matched_tag != '' AND EXISTS (
                    SELECT 1
                    FROM unnest(string_to_array(azure.matched_tag, ',')) AS matched_tag_elem
...
{%- endif -%}
INSERT INTO {{schema | sqlsafe}}.managed_reporting_ocpazurecostlineitem_project_daily_summary (
    row_uuid,
    cluster_id,
    cluster_alias,
    data_source,
...
WITH cte_rankings AS (
    ...
    FROM {{schema | sqlsafe}}.managed_reporting_ocpazurecostlineitem_project_daily_summary_temp AS pds
    GROUP BY row_uuid
)
SELECT pds.row_uuid,
    cluster_id,
...
SELECT azure.row_uuid as row_uuid,
    ...
    max(azure.year) as year,
    max(azure.month) as month,
    EXTRACT(DAY FROM max(azure.usage_start))::text as day
    FROM {{schema | sqlsafe}}.reporting_ocpusagelineitem_daily_summary_trino as ocp
    JOIN {{schema | sqlsafe}}.managed_azure_openshift_daily_temp as azure
        ON azure.usage_start = ocp.usage_start
