...
-- each hour.
INSERT INTO {{schema | sqlsafe}}.managed_aws_openshift_disk_capacities_temp (
    resource_id,
    capacity,
    usage_start,
    ocp_source,
...
WITH cte_hours as (
    SELECT EXTRACT(DAY FROM (DATE_TRUNC('month', {{start_date}}) + INTERVAL '1 month - 1 day'))::INTEGER * 24 as in_month
),
...
calculated_capacity AS (
    SELECT
        ...
        {{year}} as year,
        {{month}} as month
    FROM {{schema | sqlsafe}}.aws_line_items as aws
...
cte_csi_volume_handles as (
    SELECT distinct csi_volume_handle as csi_volume_handle
            FROM {{schema | sqlsafe}}.openshift_storage_usage_line_items_daily as ocp
...
SELECT  uuid_generate_v4()::text as row_uuid, -- need a new uuid or it will deduplicate
...
JOIN {{schema | sqlsafe}}.managed_aws_openshift_daily_temp as aws
    ...
    AND ocp.csi_volume_handle is not null
    AND ocp.csi_volume_handle != ''
JOIN {{schema | sqlsafe}}.managed_aws_openshift_disk_capacities_temp AS aws_disk
...
WITH cte_total_pv_capacity as (
    ...
    FROM (
        ...
        WHERE ocp.year = {{year}}
            ...
            AND ocp.usage_start < {{end_date}} + INTERVAL '1 day'
            AND aws.ocp_source = {{ocp_provider_uuid}}
            AND aws.year = {{year}}
            AND aws.month = {{month}}
            AND aws.resource_id_matched = True
...
SELECT  uuid_generate_v4()::text as row_uuid, -- need a new uuid or it will deduplicate
    ...
    max(aws.region) as region,
    max(aws.unit) as unit,
    NULL::double precision as usage_amount,
    max(aws.currency_code) as currency_code,
...
SELECT aws.row_uuid,
    ...
    LEFT JOIN {{schema | sqlsafe}}.managed_aws_openshift_disk_capacities_temp AS aws_disk
        ON aws_disk.usage_start = aws.usage_start
        AND aws_disk.resource_id = aws.resource_id
...
SELECT aws.row_uuid,
    ...
    JOIN {{schema | sqlsafe}}.managed_aws_openshift_daily_temp as aws
        ON aws.usage_start = ocp.usage_start
            AND (
                aws.tags::json->>'openshift_project' = ocp.namespace
                OR aws.tags::json->>'openshift_node' = ocp.node
                OR aws.tags::json->>'openshift_cluster' = ocp.cluster_alias
                OR aws.tags::json->>'openshift_cluster' = ocp.cluster_id
                OR (aws.matched_tag != '' AND EXISTS (
                    SELECT 1
                    FROM unnest(string_to_array(aws.matched_tag, ',')) AS matched_tag_elem
                    WHERE strpos(ocp.pod_labels, replace(matched_tag_elem, ' ', '')) != 0
                ))
                OR (aws.matched_tag != '' AND EXISTS (
                    SELECT 1
                    FROM unnest(string_to_array(aws.matched_tag, ',')) AS matched_tag_elem
                    WHERE strpos(ocp.volume_labels, replace(matched_tag_elem, ' ', '')) != 0
                ))
            )
...
WITH cte_rankings AS (
    ...
    FROM {{schema | sqlsafe}}.managed_reporting_ocpawscostlineitem_project_daily_summary_temp AS pds
...
SELECT pds.row_uuid,
    ...
    CASE WHEN pds.pod_labels IS NOT NULL
        THEN (COALESCE(pds.pod_labels::json, '{}'::json) || COALESCE(pds.tags::json, '{}'::json))::text
        ELSE (COALESCE(pds.volume_labels::json, '{}'::json) || COALESCE(pds.tags::json, '{}'::json))::text
    END as tags,
    aws_cost_category,
    cost_category_id,
    ...
    {{cloud_provider_uuid}} as source,
    {{ocp_provider_uuid}} as ocp_source,
    EXTRACT(YEAR FROM usage_start)::text as year,
    EXTRACT(MONTH FROM usage_start)::text as month,
    EXTRACT(DAY FROM usage_start)::text as day
FROM {{schema | sqlsafe}}.managed_reporting_ocpawscostlineitem_project_daily_summary_temp AS pds
...
SELECT
    ...
    max(EXTRACT(YEAR FROM aws.usage_start)::text) AS year,
    max(EXTRACT(MONTH FROM aws.usage_start)::text) AS month,
    max(EXTRACT(DAY FROM aws.usage_start)::text) AS day
FROM {{schema | sqlsafe}}.reporting_ocpusagelineitem_daily_summary_trino AS ocp
JOIN {{schema | sqlsafe}}.managed_aws_openshift_daily_temp AS aws
    ON aws.usage_start = ocp.usage_start
    AND strpos(aws.resource_id, ocp.resource_id) != 0
